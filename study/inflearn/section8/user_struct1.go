...
func main() {
	...
	//객체지향 -> 클래스(속성 : 멤버변수, 기능(상태 : 메소드)) : 코드 재사용성, 코드의 관리가 용이, 신뢰성이 높은 프로그래밍
	// GO는 전형적인 객체지향의 특징을 가지고 있지 않지만, 인터페이스 -> 다형성 지원, 구조체를 클래스형태의 코딩 가능
	// 객체지향의 기본 개념 -> Go에서 포함 하고 있다. -> 객체지향 프로그래밍 언어
	//상태, 메소드를 분리해서 정의 (결합성이 없음)
	//사용자 정의 타입 : 구조체, 인터페이스, 기본타입 (int, float, string...), 함수 타입으로 재정의 가능
	//구조체와 -> 메소드 연결을 통해 타 언어의 클래스 형식처럼 사용가능(객체 지향)

	//예제1
	bmw := Car{name: "520d", price: 50000000, color: "red", tax: 5000000}
	benz := Car{name: "220d", price: 60000000, color: "red", tax: 6000000}
